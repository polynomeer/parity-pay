...
class TransactionTimelineTest extends AbstractIntegrationTest {
    ...
    void setUp() {
        ...
        bankAccountId = onboardingService.linkBankAccount(memberId, "004", "110-2222-3333", Money.krw(1_000_000));
    }
    ...
    void timelineByPaymentId() {
        requestTopUp.requestTopUp(new TopUpCommand(
                memberId, walletId, bankAccountId, Money.krw(100_000), IdempotencyKey.of("timeline-topup-001")));
        ...
        assertThat(timeline.countsByKind()).containsKeys("PAYMENT", "PAYMENT_CANCELLATION", "LEDGER", "EVENT");
        ...
        assertThat(timeline.entries())
                .isSortedAccordingTo(Comparator.comparing(TransactionTimelineService.TimelineEntry::occurredAt));
    }
    ...
    void timelineIncludesExternalRecords() {
        TopUpView topUp = requestTopUp.requestTopUp(new TopUpCommand(
                memberId, walletId, bankAccountId, Money.krw(100_000), IdempotencyKey.of("timeline-topup-003")));
        drainOutbox();

        TransactionTimelineService.Timeline timeline =
                timelineService.of(topUp.topUpId().toString());

        assertThat(timeline.countsByKind()).containsKeys("TOP_UP", "LEDGER", "EXTERNAL_WITHDRAWAL", "EVENT");
    }

    @Test
    @DisplayName("존재하지 않는 식별자는 빈 타임라인을 돌려준다")
    void unknownReferenceReturnsEmptyTimeline() {
        TransactionTimelineService.Timeline timeline = timelineService.of("00000000-0000-0000-0000-000000000000");
    ...
    void invariantMetricsAreZeroWhenHealthy() {
        requestTopUp.requestTopUp(new TopUpCommand(
                memberId, walletId, bankAccountId, Money.krw(100_000), IdempotencyKey.of("timeline-topup-004")));
        approvePayment.approve(new ApprovePaymentCommand(
                memberId,
                "order-timeline-4",
                walletId,
