...
class JournalFactoryTest {
    ...
    void topUpJournalMatchesCatalog() {
        Journal journal = JournalFactory.topUpCompleted(TopUpId.generate(), BANK, USER, Money.krw(100_000), NOW);
        ...
        assertThat(journal.lines())
                .containsExactly(
                        JournalLine.debit(BANK, Money.krw(100_000)), JournalLine.credit(USER, Money.krw(100_000)));
    }

    @Test
    @DisplayName("JE-002 / ADR-009: 원분개와 역분개의 계정별 순효과는 0이다")
    void reversalNetsToZero() {
        Journal original = JournalFactory.topUpCompleted(TopUpId.generate(), BANK, USER, Money.krw(100_000), NOW);
        LedgerTransaction posted = LedgerTransaction.post(LedgerTransactionId.generate(), original, NOW);
        ...
        posted.entries()
                .forEach(entry -> net.merge(entry.accountId(), entry.signedAmountFor(Direction.DEBIT), Long::sum));
        reversal.lines()
                .forEach(line -> net.merge(
                        line.accountId(),
