...
부하·장애 실험 41종이 결함 14건(A~N)을 찾아냈고 전부 고쳤습니다(마지막 다섯은 뺀 대안인 분산락을 일부러 만들어 왜 뺐는지를 잰 실험입니다). 그 목록과 수치는
[성능·장애 보고서](reports/11-performance-failure-report-template.md)에 있습니다 — 통과한 것만이
아니라 틀렸던 가설과 측정 방법의 실수도 함께 적혀 있습니다.

**결함 14건 중 설계 검토로 문서나 코드를 읽어서 나온 것은 하나도 없습니다.** 여섯(B·C·F·J·K·L)은 "대비되어
있다"고 문서에 적혀 있던 것이었고, 하나(결함 J)는 시험이 **있었는데도** 통과했습니다 — 그 시험이 증명할 수 있는
...
`deploy/run.sh`가 이미지를 빌드해 그 모양 그대로 띄웁니다 — 두 호스트이름(`app.localhost`·`ops.localhost`),
TLS, 생성된 비밀값으로.

**검증 현황** (2026-09-16): 백엔드 **325개**, 프론트엔드 **56개**, E2E **7개**, 실패 0건.
