...
**외부 PG 호출은 격리 장치를 지납니다(2026-09-16, ADR-014).** `PgCallGuard`가 벌크헤드(동시 50)와
차단기(윈도 20·최소 10·실패율 50%·OPEN 10초)를 기본으로 켜고, 거절은 미확정이 아니라 알려진 실패로
확정됩니다. 느린 기관이 프로세스를 잠식하는 과정과 각 장치가 막는 것은 reports/11 M-019~M-023에 있습니다.
은행 호출은 아직 이 장치를 지나지 않습니다.
...
**대사는 2026-09-09에 기관 API로 옮겼습니다.** 전에는 기관의 표(`mock_bank_withdrawal`,
`mock_bank_payout`)를 같은 데이터베이스에서 직접 읽었습니다. 그러면 대사가 **절대 실패하지 않습니다** —
조회는 언제나 성공하고, 결과가 비어 있으면 "기관에 기록이 없다"가 됩니다. 실제 대사는 명세를 파일이나
API로 받고, 못 받는 날이 있습니다.

지금은 `GET /mock-bank/statements/{withdrawals,payouts}`로 받습니다. 명세를 받지 못하면 빈 목록이 아니라
`ReconciliationSourceUnavailableException`이고, 그 회차는 아무것도 기록하지 않고 멈춥니다. 빈 명세로
진행하면 우리 기록 전부가 불일치로 올라가 운영자가 존재하지 않는 문제를 놓고 보정을 검토하게 됩니다.
명세 가용성은 건별 조회 가용성과 **따로** 제어합니다 — 실제로 둘은 다른 시스템이고, "건별 조회는 되는데
일별 명세가 안 나오는" 날이 대사에는 더 위험합니다.

이 변경으로 대사 실행에서 트랜잭션 경계도 바뀌었습니다. 기록을 받는 것은 트랜잭션 밖이고
(`ReconciliationService`), 비교와 기록만 트랜잭션 안입니다(`ReconciliationTransactions`). 예전 구조
그대로 두면 DB 트랜잭션을 연 채 기관의 응답을 기다리게 됩니다.

**데이터 경계도 2026-09-09에 끝냈습니다.** 기관은 이제 자기 데이터베이스(`paritypay_bank`,
`paritypay_pg`)와 자기 마이그레이션(`db/mock-bank`, `db/mock-pg`)을 가집니다. pay-api의 V19가 우리
데이터베이스에서 기관 표를 내보냈습니다.

같은 컨테이너를 쓰더라도 데이터베이스가 다르면 **조인이 불가능합니다.** 그것이 요점입니다 — 예전에는
우리 쪽 코드가 마음만 먹으면 기관의 표를 읽을 수 있었고, 실제로 대사와 타임라인이 그렇게 하고
있었습니다. 실제 기관에서는 할 수 없는 일이며, 할 수 없어야 그 경로가 코드에 정직하게 드러납니다.

| 예전 | 지금 |
|---|---|
| 대사가 `mock_bank_withdrawal`을 조인 | 명세 API로 받고, 못 받으면 회차를 멈춤 |
| 타임라인이 기관 표를 SELECT | 건별 조회 API. 물어보지 못하면 그 사실을 줄로 표시 |
| 시험이 우리 JdbcTemplate으로 기관 표를 TRUNCATE | 기관의 `/admin/reset`에 부탁 |

**타임라인이 기관을 못 부르는 경우**는 조용히 빼지 않습니다. 빼면 운영자가 "기관에 기록이 없다"로
읽습니다 — 대사에서 배운 것과 같은 구분입니다(F-011). 우리 쪽 사실은 그대로 보여 주고 기관 부분만
`UNAVAILABLE`로 표시합니다. 기관이 답하지 않는다고 타임라인 전체를 실패시키지도 않습니다.

**시험이 기관 장부를 손보는 통로**(`/admin/reset`, `/admin/withdrawals/amend`, `.../orphan`)는 기관
쪽에 있습니다. 이 앱들은 운영에 배포되지 않습니다. 우리가 남의 장부에 직접 쓸 수 없다는 사실이
시험 코드에도 그대로 나타납니다.
...
- **전달은 SMTP입니다** (2026-09-13). `spring.mail.host`가 설정되면 `SmtpPasswordResetDelivery`가
  켜지고 링크 템플릿(`paritypay.password-reset.link-template`, `{token}` 자리 필수)으로 메일을
  보냅니다. 설정이 없으면 예전처럼 요청 사실만 남기고 토큰을 버립니다 — 알려진 채널 없이 조용히
  뜨지 않는다는 뜻입니다.
  - **전달은 커밋 뒤입니다.** SMTP는 외부 호출이고 트랜잭션 안에서 외부 호출을 하지 않습니다.
    커밋 전에 보내면 롤백된 토큰이 담긴 메일이 나갑니다. 메일이 실패하면 토큰은 있지만 아무도 갖지
    못한 상태가 되는데, 다시 요청하면 살아 있는 토큰을 무효화하고 새로 만들므로 남는 위험이 없습니다.
  - 로컬·확인용 스택은 Mailpit이 SMTP를 받습니다. E2E가 실제로 나간 메일에서 링크를 꺼내 새
    비밀번호를 정하고 로그인까지 갑니다 — 백엔드 시험(전달 목)과 프론트 시험(API 목) 둘 다 볼 수
    없는 경로입니다.
...
배포 형태는 [ADR-011](adr/011-deployment-shape.md)에 있습니다. 요점은 **두 앱이 각자 자기 오리진에서
`/api`를 `pay-api`로 프록시하고, `pay-api`는 바깥에 열리지 않는다**는 것입니다. 그래서 CORS 설정이
없습니다 — 교차 오리진 요청이 생기지 않기 때문입니다.

- `local`, `test`, `stage` 프로필을 분리합니다. **배포는 프로필 없이** 띄웁니다. `local`에만 있는
  개발용 비밀값과 느슨한 쿠키 설정에 닿지 않게 하기 위해서입니다.
...
- 구성과 비밀값은 이미지 밖에서 주입합니다. `PARITYPAY_JWT_SECRET`과 `PARITYPAY_WEBHOOK_SECRET`은
  **없으면 애플리케이션이 뜨지 않습니다.** 기본값을 두면 배포가 알려진 값으로 조용히 뜹니다 —
  실제로 웹훅 비밀값이 그런 상태였고 ADR-011을 구현하다 드러났습니다.
...
- 단일 DB 잠금·커넥션 경합이 측정 가능한 병목입니다. **2026-09-14에 쟀습니다(reports/11 M-013):**
  경합 없는 부하에서 초당 약 700 승인까지 잠금 대기는 0%였고, DB 백엔드는 시간의 75~84%를 앱을
  기다리며 보냈습니다. 먼저 막힌 것은 앱의 커넥션 풀이었습니다. 이 조건은 아직 성립하지 않습니다.
