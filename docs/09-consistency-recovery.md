...
- `PENDING`이면서 `next_attempt_at <= now()`인 이벤트를 선택합니다.
- 여러 발행기가 `FOR UPDATE SKIP LOCKED`로 나눠 처리합니다.
...
- 배치가 가득 차서 돌아오면 다음 폴링을 기다리지 않고 이어서 비웁니다(한 번에 최대
  `max-rounds-per-poll` 배치). 그렇지 않으면 폴링 간격이 상한이 됩니다(배치 100 × 초당 2회 =
  200건/초). 배치마다 트랜잭션은 따로 끝납니다.
