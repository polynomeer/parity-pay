...
### 처리할 수 없는 레코드 (결함 M, 2026-09-16)

소비자가 재시도를 다 쓰고도 처리하지 못한 레코드는 **조용히 건너뛰지 않습니다.** 고치기 전에는
spring-kafka 기본 핸들러가 0 ms 간격으로 10회 재시도한 뒤 ERROR 로그 한 줄만 남기고 오프셋을 넘겼고,
그 경로를 우리 이벤트가 역직렬화 버그로 타면 거래내역·정산 효과가 빠지는데 원장은 맞으므로 불변조건
지표도 대사도 잡지 못했습니다(reports/11 M-018).

지금은 다음 순서입니다.

1. 일시적 오류(DB·네트워크)는 1초 간격으로 두 번 더 시도합니다. 파싱 실패(`JacksonException`)는
   재시도하지 않습니다 — 같은 바이트는 몇 번 읽어도 같고, 재시도는 그 파티션을 그만큼 더 멈춥니다.
2. 포기한 레코드는 `dead_letter_event`에 한 줄(그룹·토픽·파티션·오프셋 유니크)로 남기고
   `paritypay.events.dlt`로 발행해 **확인을 받은 뒤에야** 원 토픽의 오프셋을 넘깁니다. 둘 중 하나라도
   실패하면 오프셋을 넘기지 않고 다시 시도합니다 — 어디에도 남기지 못한 레코드를 버리는 것보다
   파티션이 멈추는 편이 낫습니다.
3. `paritypay.consumer.dead_letters`(카운터)·`paritypay.consumer.dead_letters_open`(게이지)가 오르고,
   게이지가 0이 아니면 불변조건과 같은 등급으로 즉시 경보합니다.
4. 운영자는 `GET /admin/dead-letters`로 보고 `POST /admin/dead-letters/{id}/retry`로 **같은 키·같은
   payload를 원 토픽에 다시 발행**합니다(감사 로그·사유 필수). 소비자가 멱등이므로 이미 처리된 것이
   섞여도 효과는 한 번이고, 코드를 고치지 않은 채 되돌리면 새 DLT 행이 생깁니다. 내용을 고쳐서 보내는
   기능은 없습니다 — 그것은 이벤트 위조입니다.
...
### 보내지 않은 요청은 미확정이 아닙니다 (ADR-014)

외부 호출 앞의 격리 장치(벌크헤드·차단기·리미터, `PgCallGuard`)가 요청을 거절하면 요청은 기관에 나가지
않은 것입니다. 그것은 `PgCallRejectedException`으로 구분되고 결제·취소는 `FAILED`(사유 `CIRCUIT_OPEN`·
`BULKHEAD_FULL`·`RATE_LIMITED`)로 즉시 확정됩니다. 미확정으로 두면 복구 작업이 존재하지 않는 기록을
세 번씩 묻습니다. 타임아웃과 구분하는 기준은 하나 — **바이트가 소켓을 떠났는가**입니다.
...
기관이 결과를 먼저 알려주는 경로입니다. 인증 토큰 없이 열려 있는 유일한 쓰기 경로이므로 다음을
지킵니다.

1. **서명으로 신원을 확인합니다.** 나눠 가진 비밀로 만든 HMAC이며, 서명 대상은 시각과 **본문
   원문**입니다. 파싱 후 재직렬화한 것에 서명하면 공백 하나에 검증이 깨지고, 반대로 파서가 무시하는
   필드를 끼워 넣을 수 있습니다.
2. **payload를 믿지 않습니다.** "승인됐다"는 말로 돈을 움직이지 않고 기관에 직접 물어보는 복구
   경로를 촉발합니다. 확정 경로가 하나여야 확인할 곳도 하나입니다. 웹훅의 값어치는 정확성이 아니라
   **속도**입니다 — 복구 주기를 기다리지 않고 그 자리에서 확정됩니다.
3. **중복은 `webhook_receipt`가 막습니다.** `(provider, event_id)`에 `ON CONFLICT DO NOTHING`이며,
   재전송은 같은 `eventId`로 옵니다.
4. **역순은 `webhook_cursor`가 막습니다.** 업무 키별 순번을 전진만 시키는 조건부 UPDATE 한 문장입니다.
   도착 시각으로는 순서를 알 수 없고, 기관이 적은 시각은 재전송 때 그대로 옵니다.
