...
- [x] 결제 승인에서 차감 직전 명시적 flush (2026-09-10, M-010) — 처리량으로는 두 번 다 판정하지
      못했습니다(부하 평균 34.8). **문장 순서**로 바꾸니 한 번에 나왔습니다 — flush가 Hibernate
      쓰기 네 개를 잠금 밖으로 내보내 보유가 **26 ms → 1 ms**입니다. 순서는 CPU 경합에 흔들리지
      않습니다. 결과는 reports/11 M-010
