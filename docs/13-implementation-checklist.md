...
- ~~**외부기관 프로세스 분리**~~: Mock Bank·Mock PG 모두 완료(2026-09-08). 대사도 기관 명세 API로
  옮겼습니다(2026-09-09) — 전에는 기관 표를 같은 DB에서 읽어 대사가 실패할 수 없었습니다. 남은 결합은
  기관이 아직 우리 데이터베이스를 공유하고 타임라인이 기관 표를 직접 읽는다는 것입니다(docs/05 §10).
