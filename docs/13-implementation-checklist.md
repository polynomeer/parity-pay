...
- [ ] Cancellation UNKNOWN — 외부 PG **환불**을 붙일 때 활성화합니다. 지금 취소는 페이머니
      역분개만 하므로 외부 호출이 없습니다
...
- **외부 PG 환불**: 승인은 붙였고(JE-013, `PaymentRecoveryService`) 취소는 아직 페이머니 역분개만
  합니다. 환불을 붙이면 취소의 `UNKNOWN` 경로와 F-007이 열립니다.
