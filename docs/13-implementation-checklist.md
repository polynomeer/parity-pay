...
- [x] **부하 실험 실행** (2026-09-07) — P-001·P-002·P-003 실측. 결과와 발견한 결함 2건은
      reports/11에 있습니다. P-004(정산·API 혼합)는 아직 미작성입니다
- [ ] 속성 기반 테스트(jqwik)로 임의 거래 시퀀스 검증
- [ ] 스냅샷 손상 후 원장 재구축 실험 (F-010)
- [ ] 프로세스 강제 종료 기반 F-001·F-002 재현
...
- [ ] 판매자(`MERCHANT`) 역할 활용 — 정산 조회가 아직 관리자 경로에만 있습니다
- [ ] 비밀번호 변경·재설정 흐름
...
- **P-004 부하 시나리오**: 정산 배치와 API 부하를 동시에 거는 시나리오는 아직 없습니다.
- **Outbox 발행 처리량**: 80건/초로 결제 처리량과 여유가 크지 않습니다. 배치 전송으로 개선할 수
  있지만 ACK 확인 시점은 유지해야 합니다.
