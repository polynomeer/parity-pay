...
요청은 `200 OK`와 같은 본문입니다 — 요청 처리는 성공했고 결과가 실패입니다(4xx가 아닙니다). 결제 응답의
`failureReason`은 기관의 거절 코드이거나, 격리 장치가 보내지 않고 거절한 이유(`CIRCUIT_OPEN`·`BULKHEAD_FULL`·
`RATE_LIMITED`, ADR-014)입니다.
