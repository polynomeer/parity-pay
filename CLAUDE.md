...
- `main`은 배포 가능한 상태를 유지하며 직접 커밋하지 않습니다. 작업은 `<type>/<간단한-설명>`
  브랜치에서 하고, 브랜치 이름을 사용자에게 알립니다. 병합은 사용자가 합니다.
  - 예외: 저장소에 커밋이 하나도 없던 초기 이력(2026-09-07)은 `main`에 직접 쌓았습니다.
    비어 있는 기본 브랜치를 두고 기능 브랜치만 만드는 것이 더 이상했습니다.
